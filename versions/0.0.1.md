...
---|---|---|---
GET| Fetches an integer entry. This will fail if the integer entry does not yet exist| None|```json **YAML**
responses:
  '200':
...
| POST| Updates an existing integer entry. This will fail if the integer entry does not yet exist| **YAML**
```json
required: true
  content:
    application/json:
